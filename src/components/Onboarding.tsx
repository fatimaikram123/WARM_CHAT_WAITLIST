...
const presets = [
  {
    id: "buyer-follow-up",
    title: "Buyer Lead Follow-Up",
    recommended: true,
    avgReplies: "32–45%",
    messages: "3 (Day 0, Day 1, Day 3)",
  },
  {
    id: "seller-follow-up",
...
const automationFlows = [
  {
    id: "seller-lead-follow-up",
    title: "Seller Lead Follow-Up",
    description: "Re-engages homeowners asking about value, selling options, or timelines.",
    avgReplies: "25–35%",
    messages: "3",
    topAgent: true,
  },
  {
    id: "open-house-follow-up",
    title: "Open House Follow-Up",
    description: "Automatically follows up with visitors after open houses.",
    avgReplies: "40–55%",
    messages: "2",
    topAgent: true,
  },
  {
    id: "expired-listing-outreach",
    title: "Expired Listing Outreach",
    description: "Starts conversations with homeowners whose listings didn’t sell.",
    avgReplies: "18–25%",
    messages: "4",
    topAgent: true,
  },
  {
    id: "meta-lead-auto-reply",
    title: "Meta Lead Auto-Reply",
    description: "Instant auto-reply when someone submits a Facebook or Instagram lead form.",
    avgReplies: "50–65%",
    messages: "2",
    topAgent: true,
  },
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <div className="h-2 rounded-full bg-gray-200 overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-purple-600 to-orange-500 transition-all"
        ...
        <Card className="shadow-xl border border-white/40 bg-white/70 backdrop-blur-md">
          ...
          <CardContent>
            ...
            {step === 1 && (
              <button
                onClick={() => setStep(2)}
                className="px-6 py-3 rounded-lg font-semibold text-white bg-gradient-to-r from-purple-600 to-orange-500 hover:opacity-90 shadow-lg"
              >
                Get Started
              </button>
            )}
            ...
            {step === 2 && (
              <div className="grid sm:grid-cols-2 gap-4">
                <GoalCard
                  label="🏡 Real Estate — Get More Clients"
                  recommended
                  selected={goal === "Real Estate — Get More Clients"}
                  onSelect={() => saveGoal("Real Estate — Get More Clients")}
                />
                {["📧 Cold Email Campaigns", "🔁 Lead Nurturing"].map((g) => (
                  <GoalCard
                    key={g}
                    label={g}
                    selected={goal === g}
            ...
            {step === 3 && (
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <ChannelCard
                  ...
                  onConnect={() => setConnected({ ...connected, sms: true })}
                />
              </div>
            )}
              ...
              <div className="space-y-4">
                {presets.map((p) => (
                  <Card key={p.id} className="p-4 flex justify-between items-center">
                    ...
                    <button
                      onClick={() => activateFlow(p.id)}
                      className="px-4 py-2 rounded-md text-white bg-gradient-to-r from-purple-600 to-orange-500 hover:opacity-90"
                    >
                      Activate
                    </button>
                  </Card>
...
{step === 5 && (
  <div className="space-y-4">
    <button
      onClick={sendTest}
      className="w-full px-6 py-3 rounded-lg font-semibold text-white bg-gradient-to-r from-purple-600 to-orange-500 hover:opacity-90"
    >
      Send Test Message
    </button>
    ...
    <button
      onClick={async () => {
        ...
        if (res.ok) alert("Sample lead created!");
        else alert("Failed to create lead.");
      }}
      className="w-full px-6 py-3 rounded-lg font-semibold text-white bg-gradient-to-r from-purple-400 to-orange-400 hover:opacity-90"
    >
      Create Sample Lead
    </button>
  </div>
          ...
          <CardFooter className="flex justify-between">
            ...
            {step < 6 && step > 1 && (
              <button
                ...
                disabled={!canNext()}
                className="px-4 py-2 rounded-md text-white bg-gradient-to-r from-purple-600 to-orange-500 disabled:opacity-40"
              >
                Continue
              </button>
            )}
...
const GoalCard = ({
  ...
  <button
    ...
    className={`relative w-full text-left p-5 rounded-xl border transition-all
      ${selected
        ? "border-transparent bg-gradient-to-r from-purple-600/10 to-orange-500/10"
...
const EmailConnectModal = ({ onClose, onSuccess }: any) => {
  ...
  const connect = async () => {
    ...
    const res = await fetch(`${API_BASE}/inbox/connect`, {
      ...
      body: JSON.stringify({ email, password,   imap_host: "imap.gmail.com", smtp_host: "smtp.gmail.com", user_id:user_id }),
    });
    if (res.ok) {
      setStatus("Connected!");
      onSuccess();
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl p-6 w-full max-w-md space-y-4 relative">
        ...
        <button onClick={connect} className="w-full py-2 rounded text-white bg-gradient-to-r from-purple-600 to-orange-500">
          Connect
        </button>
